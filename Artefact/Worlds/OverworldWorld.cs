    ...
    internal class OverworldWorld : World
    {
        ...
        private void GenerateGrass()
        {
            for (int y = 0; y < Height; y++)
            {
                ...
                {
                    SetTile(new Vector2(x, y), Tile.GrassTile);
                }
            }

            for (int i = 0; i < Random.Next(40, 60); i++)
            {
                Vector2 pos = Random.NextVector2i(new Vector2(Width, Height));
        ...
        private void GenerateFlowers()
        {
            ...
            {
                Vector2 pos = GetRandomTilePos(Tile.GrassTiles.ToArray());
        ...
        private void GenerateTrees()
        {
            ...
            while (i < amountTrees)
            {
                ...
                {
                    if (!CheckTilesAroundSame(pos + new Vector2(j % size, j / size), true, Tile.GrassTiles.ToArray()))
                    {
                        cont = true;
                        break;
                    }
                ...
                for (int j = 0; j < size * size; j++)
                {
                    SetTile(pos + new Vector2(j % size, j / size), Tile.TreeBarkTile);
                }
        ...
        private void GenerateCaves()
        {
            ...
            while (i < amountCaves)
            {
                ...
                for (int j = 0; j < size * size; j++)
                {
                    CaveTile caveTile = SetTile(pos + new Vector2(j % size, j / size), Tile.CaveTile);
                    if (caveTile != null)
